...
// Arduino String helper functions
String decodeBase64(const String& input) {
  if (input.length() == 0) return "";
  
  unsigned int output_length = decode_base64_length((const unsigned char*)input.c_str(), input.length());
  unsigned char* output = new unsigned char[output_length + 1];
  
  decode_base64((const unsigned char*)input.c_str(), input.length(), output);
...
String encodeBase64(const String& input) {
  ...
  unsigned int output_length = encode_base64_length(input.length());
  unsigned char* output = new unsigned char[output_length + 1];
  
  encode_base64((const unsigned char*)input.c_str(), input.length(), output);
  output[output_length] = '\0';
